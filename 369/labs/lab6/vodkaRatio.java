...
public void map(Text key, Text value, Context context)
      ...
      String vodkaBottles = "0";
      String vodkaDollars = "0";
      String itemDescription = record[record.length - 3];
      if (itemDescription.toLowerCase().contains("vodka")){
        vodkaBottles = record[record.length - 2];   
        vodkaDollars = record[record.length - 1];   
      }
      Text outKey = county;
      Text outVal = new Text(totalBottles + " " + vodkaBottles + " " + totalDollars + " " + vodkaDollars);
...
public void reduce( Text key, Iterable<Text> values, Context context)
    ...
    float vodkaBottles = 0;
    float totalDollars = 0;
    float vodkaDollars = 0;
  for (Text val : values) {
    totalBottles = totalBottles + Float.parseFloat(val.toString().split(" ")[0]);
    vodkaBottles = vodkaBottles + Float.parseFloat(val.toString().split(" ")[1]);
    totalDollars = totalDollars + Float.parseFloat(val.toString().split(" ")[2]);
    vodkaDollars = vodkaDollars + Float.parseFloat(val.toString().split(" ")[3]);
  }
  Text result = new Text("Total Bottles: " + Float.toString(totalBottles) + " Percent Vodka Bottles: " + Float.toString((vodkaBottles/totalBottles) * 100 ) + " Total Dollars: " + Float.toString(totalDollars) + " Percent Vodka Dollars: " + Float.toString((vodkaDollars/totalDollars)* 100  ) );
...
//  MapReduce Driver
  ...
  public static void main(String[] args) throws Exception {
    ...
    // step 2: register the MapReduce class
      job.setJarByClass(vodkaRatio.class);  
