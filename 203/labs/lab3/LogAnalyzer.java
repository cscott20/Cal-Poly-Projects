...
public class LogAnalyzer
{
    ...
    private static void processStartEntry(
        final String[] words, //line in array form
        final Map<String, List<String>> sessionsFromCustomer)
    {
        ...
        //sessionsFromCustomer[words[START_CUSTOMER_ID]] = sessions; //is this right to be added?
    }

        //similar to processStartEntry, should store relevant view
        //data in a map - model on processStartEntry, but store
        //your data to represent a view in the map (not a list of strings)u
    ...
    private static void processViewEntry(final String[] words, final Map<String, List<View>> viewsFromSession)
    {
        ...
        View view = new View(new String (words[VIEW_PRODUCT_ID]), new Integer (words[VIEW_PRICE]));   
        //now that we know there is a list, add the current session
        //view.incrementViews()
        views.add(view);
        //sessionsFromCustomer[words[START_CUSTOMER_ID]] = sessions; //is this right to be added?
        ...
        
    {
        ...
        if (buys == null)
        {
            ...
            //now that we know there is a list, add the current session
        Buy buy = new Buy(new String (words[BUY_PRODUCT_ID]), new Integer (words[BUY_PRICE]), new Integer(words[BUY_QUANTITY]));   
        //now that we know there is a list, add the current session
        //view.incrementViews()
        buys.add(buy);
    }
    ...
    private static void printAverageViewsNoPurchase(
        final Map<String, List<String>> sessionsFromCustomer,
        final Map<String, List<View>> viewsFromSession,
        final Map<String, List<Buy>> buysFromSession)
    {
        ...
        for (Map.Entry<String, List<String>> entry : sessionsFromCustomer.entrySet())
        {
            for (String session : entry.getValue())
            {
                ...
                if (viewsFromSession.get(session) != null){
                    viewcount += viewsFromSession.get(session).size();
        ...
        double average = viewcount / sesscount; 
        ...
        final Map<String, List<Buy>> buysFromSession)
    {
        ...
        {
            String sess = new String (entry.getKey());
            System.out.println(sess);
            double sum = 0;
            double count = 0;
            for (View view : viewsFromSession.get(sess))
            {
                count ++;
                sum += view.getPrice();
            }  
            double avg = sum / count;     
            for (Buy buy : entry.getValue())
            {
                System.out.print("\t");
                System.out.print(buy.getProductID());
                System.out.print(" ");
                System.out.print(buy.getPrice() - avg);
                System.out.println();
            }
...

    {
        ...
        {
            printed = false;
            String cust = new String (entry.getKey());
            for (String session : entry.getValue()) //iterating through each session for a customer
            {
                if (buysFromSession.get(session) != null) // if there was a buy in that session
                {
                    for (Buy buy : buysFromSession.get(session)) //go through every buy in that session
                    {
                            count = 0;
                if (!printed){
            System.out.println(cust);
                    }
                        for (String session2 : entry.getValue()) // now we know what product was bought, need to count how many times it was viewed by customer
                        {
                            b = false;
                            if (viewsFromSession.get(session2) != null)
                            {
                            for (View view : viewsFromSession.get(session2)) // go through every view for that session where something was bought
                            {
    ...
    private static void printStatistics(
        ...
        final Map<String, List<Buy>> buysFromSession)
    {
        printAverageViewsNoPurchase(sessionsFromCustomer, viewsFromSession, buysFromSession);
